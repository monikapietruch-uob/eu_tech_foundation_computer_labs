...
async function boot() {
  try {
    ...
    var karelSource = await (await fetch(new URL("karel-api.py?v=5", self.location.href))).text();
    pyodide.runPython(karelSource);
    karelRun = pyodide.globals.get("_hub_run_karel");
    // call_gpt() for the AI task (week 9).
    var aiSource = await (await fetch(new URL("ai-api.py?v=5", self.location.href))).text();
